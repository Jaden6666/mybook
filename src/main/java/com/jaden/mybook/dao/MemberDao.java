...
public class MemberDao {
    ...
    //不改金额
    public int modify(long id,String name,String pwd,long typeId,String tel,String idNumber) throws SQLException {
        Connection conn = DBHelper.getConnection();
        String sql="update member set `name` = ?, pwd=?,typeId=?,tel=?,idNumber = ? " +
                "where id=?";
        int count = queryRunner.update(conn,sql,name,pwd,typeId,tel,idNumber,id);
        DBHelper.close(conn);
        return count;
    }
